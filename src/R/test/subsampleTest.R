...
packages = c("tidyverse", "futile.logger", "DESeq2", "vsn", "modeest", "csaw", "checkmate", "limma", "tools", "methods", "lsr", "ggrepel", "jsonlite", "foreach", "doParallel", "purrr", "reshape2", "viridis", "scales")
...
# args = c()
# args[1] = "/scratch/carnold/CLL/TF_act_downsampling0.125"             
# args[2] = "/scratch/carnold/CLL/TF_act_downsampling0.125/output/FINAL_OUTPUT/extension100/subsamplingSamples"              
# args[3] = "6_6"

if (length(args) != 4) {
  stop("Expecting 4 arguments but found ", length(args),". Exiting.")
} else {
  par.l$rootFolder       = args[1]
  par.l$outputFolder     = args[2]
  par.l$subsamplesIndex  = args[3]
  repetitionsMax         = as.numeric(args[4])
}



# Because in the config file, we tried 15 in the end. THis value is parsed from the con fig file directly.
extensionSizeForced  = 100
...
par.l$file_output_log            = paste0(par.l$rootFolder, "/output/Logs_and_Benchmarks/subsample", par.l$subsamplesIndex, ".R.log")
...
# TEMPORARY SOLUTION:
extensionSize = extensionSizeForced
...
par.l$file_outputSummary = paste0(par.l$outputFolder, "/", analysisName, ".subsample", par.l$subsamplesIndex , ".StatsAllTF.tsv")
...
subsamples.l = permuteSampleTable(file_sampleData, conditionComparison, "Treatment", par.l$stepsize, repetitionsMax)

assertSubset(par.l$subsamplesIndex, names(subsamples.l))
subsamples.l = list(subsamples.l[[par.l$subsamplesIndex]])
names(subsamples.l) = par.l$subsamplesIndex
...
# TODO: This does not make sense (or?) and the code did not work. Check now
batchesPerm = sapply(subsamples.l, function(x) { sort(unique(x$Treatment))})
stopifnot(length(batchesPerm) > 1)
...
for (subsampleCounter in 1:length(subsamples.l)) {
  ...
  for (TFCur in allTF) {
    ...
    res_DESeq  = tryCatch( {
      DESeq(TF.cds.filt, fitType = 'local', quiet = TRUE)
      
    }, error = function(e) {
      warning("Warning: Could not run DESeq with local fitting, retry with default fitting type...")
      DESeq(TF.cds.filt, quiet = TRUE)
    }
    )
  ...
  file_allMotifs = paste0(par.l$outputFolder, "/", analysisName, ".subsample", nameSubsampleCur , ".allMotifs.tsv")
  ...
  system(commandCall)
   ...
   flog.info(paste0(" Permute and summarize"))
    ...
    for (TFCur in allTF) {
      ...
      data.df[TFCounter, "TF"]   = TFCur
      
      data.df[TFCounter, "TFBS"] = nRowsCurTF
      
      data.df[TFCounter, "weighted_mean"] = weighted.mean(perm.l[[nameSubsampleCur]][[TFCur]][[nameCur]]$mean ,
                                                                    perm.l[[nameSubsampleCur]][[TFCur]][[nameCur]]$ratio_TFBS, na.rm = TRUE)
      data.df[TFCounter, "weighted_Tstat"] = weighted.mean(perm.l[[nameSubsampleCur]][[TFCur]][[nameCur]]$pval,
                                                                     perm.l[[nameSubsampleCur]][[TFCur]][[nameCur]]$ratio_TFBS, na.rm = TRUE)
      data.df[TFCounter, "weighted_CD"] = weighted.mean(perm.l[[nameSubsampleCur]][[TFCur]][[nameCur]]$cohensD,
                                                                  perm.l[[nameSubsampleCur]][[TFCur]][[nameCur]]$ratio_TFBS, na.rm = TRUE)
      data.df[TFCounter, "weighted_median"] = weighted.mean(perm.l[[nameSubsampleCur]][[TFCur]][[nameCur]]$median,
                                                                      perm.l[[nameSubsampleCur]][[TFCur]][[nameCur]]$ratio_TFBS, na.rm = TRUE)
      
      
      data.df$Cohend_factor = ifelse(data.df$weighted_CD < par.l$thresholds_CohensD[1], par.l$classes_CohensD[1], 
                                               ifelse(data.df$weighted_CD < par.l$thresholds_CohensD[2] , par.l$classes_CohensD[2], 
                                                      ifelse(data.df$weighted_CD < par.l$thresholds_CohensD[3], par.l$classes_CohensD[3], par.l$classes_CohensD[4])))
      
      data.df$Cohend_factor = factor(data.df$Cohend_factor, levels = par.l$classes_CohensD, labels = seq_len(length(par.l$classes_CohensD)))

      # Concetanate all motifs from all TF
      if (is.null(nrow(permSummaryAll.df))) {
        permSummaryAll.df = data.df[1,]
      } else { 
        permSummaryAll.df = rbind(permSummaryAll.df, data.df[1,])
      }
    ...
    filename = paste0(par.l$outputFolder, "/", analysisName, ".all.volcano.l2FC.removedCGbias.AR.subsample", nameSubsampleCur , ".pdf")
    ...
    file_StatsAllTFs = par.l$file_outputSummary
    write_tsv(permSummaryAll.df, path = file_StatsAllTFs, col_names = TRUE)
    
   
    
    flog.info("  Finished with subsample \n")

  
} # end loop through all subsamples

# TODO: The results should be conetanated, see the script summarizeSubsamplingAll.R
